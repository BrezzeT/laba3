...
public class RoomController {
    ...
    public String executeQuery(@RequestParam("sqlQuery") String sqlQuery, Model model) {
        try {
            // Додайте перевірку на "BAD GRAMMAR"
            if (sqlQuery.toUpperCase().contains("DROP") || sqlQuery.toUpperCase().contains(";")) {
                throw new IllegalArgumentException("BAD GRAMMAR: SQL query contains forbidden keywords or characters.");
            }
